...
```
python .claude/scripts/graph_query.py stats
python .claude/scripts/graph_query.py callers "valider_commande"
python .claude/scripts/graph_query.py path "api" "base_donnees"
python .claude/scripts/graph_query.py schema      # si un résultat semble vide à tort
```
...
Claude Code n'a pas d'outil de recherche KiCad, et les librairies KiCad vivent
...
```
python .claude/scripts/kicad_search.py --index
```
...
```
python .claude/scripts/kicad_search.py footprint "SOIC-8"
python .claude/scripts/kicad_search.py symbol "ESP32-C3"
```
...
`.claude/scripts/pcbparts.py` (niveau 3 : modèles ECAD et tarifs depuis
...
recours seulement. Voir aussi `.claude/scripts/kicad_fetch_part.py` (extraction d'un symbole des librairies
...
```
python .claude/scripts/cq_check.py pieces/boitier.py --attendu 120x60x40
```
