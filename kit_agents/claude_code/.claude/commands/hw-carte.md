...
- Vérifier la section `## 5. Assumptions & Open Questions` : si la tension d'alimentation, les interfaces, les broches ou le budget sont flous, `AskQuestion` MAINTENANT.
- Identifiant de mission (`AAAA-MM-JJ-slug`), annoncé.
- `LS` sur `data_sheets/` : quels composants sont documentés ? Un composant
  attendu sans datasheet est un risque à signaler dès maintenant.
- `LS` sur `.agent_reports/` : écarter explicitement les rapports périmés.
