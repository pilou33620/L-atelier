...
```
run_command: python .claude/scripts/kicad_search.py symbol "ESP32-C3"
```
...
2. Exécution du script du circuit via
   `python .claude/scripts/run_projet.py circuits/<nom>.py`, qui déclenche `ERC()` et
...
```
run_command: python .claude/scripts/kicad_fetch_part.py chercher "<référence>"
run_command: python .claude/scripts/kicad_fetch_part.py copier "<Librairie>:<Nom>"
```
...
```
run_command: python .claude/scripts/pcbparts.py disponible "<référence>"
run_command: python .claude/scripts/pcbparts.py installer "<référence>" --projet .
```
