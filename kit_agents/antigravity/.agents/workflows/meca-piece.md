...
- **Vérifier qu'on a les cotes.** Consulter la section `## 5. Assumptions & Open Questions` : dimensions, jeux fonctionnels, méthode de fabrication, contraintes d'assemblage. Si ça manque, `ask_question` MAINTENANT : une cote inventée donne une pièce qui ne s'assemble pas.
...
`.agents/scripts/cq_check.py` et compare les dimensions obtenues au cahier des
charges.
