...
Les commandes exactes de ce projet sont déclarées dans `AGENTS.md`, section
« Commandes de vérification ». **C'est la seule source autorisée.**

Procédure :

1. Lis la section « Commandes de vérification » de `AGENTS.md`.
...
3. Si la section est vide ou incomplète, ne devine PAS une chaîne d'outils.
   ...
   réponse finale que `AGENTS.md` devrait être complété.
...
Si `AGENTS.md` déclare une commande de formatage, ne la lance jamais
